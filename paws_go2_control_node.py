...
from go2_interfaces.msg import Go2State, IMU, Go2Move, Go2RpyCmd
...
class Go2ControlNode(Node):
    def __init__(self):
        ...
        self.cmd_vel_publisher = self.create_publisher(Twist, '/cmd_vel_out', 10)
        ...
        self.move_subscriber = self.create_subscription(Go2Move, '/move', self.move, 10)
        self.rotate_subscriber = self.create_subscription(Go2RpyCmd, '/rotate', self.rotate, 10)
    ...
    def move_msg_callback(self):
        ...
        if self.is_moving():
            for axis in v.keys():
                ...
                self._logger.info(f"{axis}: {dist}")        
                if dist < GOAL_EPSILON[axis]:
                    v[axis] = 0.0
                else:
                    v[axis] = MAX_VEL[axis] if dist > 1 else MIN_VEL[axis]
                self._logger.info(f"{axis}: {v[axis]}")        
        ...
        # self.get_logger().info(f"{move_this_frame}, {self.is_moving()}")
        ...
        msg = Twist()
        msg.linear.x = float(v['x'])
        msg.linear.y = float(v['y'])
        msg.angular.z = float(v['yaw'])
        self.move_msg = msg
